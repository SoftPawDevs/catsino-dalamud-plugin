...
- It manages sessions, invites, player roster views, dealer actions, payout execution, and — for the table games — the **live dealer table** on the session's **Table** sub-tab (Blackjack: Deal / Hit / Stay; Hold'em: Deal; Roulette: Spin).
- **Four game types** can be created: `plinko`, `blackjack`, `holdem` and `roulette`. Plinko is instant; Blackjack is the turn-based table the dealer plays a hand at; Texas Hold'em is player-versus-player, so the dealer only starts each hand and the backend runs the streets; Roulette is a shared round on a fair European wheel where the dealer only releases the ball. The game type is chosen in the create-session UI and sent as `CreateGameSessionRequest.GameType`.
...
- Blackjack table UI: `src/Catsino.Plugin/Ui/BlackjackPanelRenderer.cs`; Hold'em table UI: `src/Catsino.Plugin/Ui/HoldemPanelRenderer.cs`; Roulette table UI: `src/Catsino.Plugin/Ui/RoulettePanelRenderer.cs`; shared card images: `src/Catsino.Plugin/Ui/CardTextures.cs`; wheel art: `src/Catsino.Plugin/Ui/RouletteTextures.cs`
- Settle & remove (booking a payout the dealer made outside the game): `src/Catsino.Plugin/Ui/SessionPanelRenderer.cs` + `CatsinoRuntime.SubmitManualSettlementAsync`
...
- The plugin must not become an authoritative balance or game-outcome source. For Blackjack it renders the server's table projection and submits dealer Deal/Hit/Stay; for Hold'em it renders the projection and submits Deal only; for Roulette it renders the shared table and submits Spin only. It never deals cards, values hands, spins a wheel, checks a bet's legality, or splits a pot locally.
