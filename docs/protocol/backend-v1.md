...
- `POST /api/v1/payout-operations/{operationId}/reconcile`

Every financial mutation sends a UUID `Idempotency-Key`: session create/fee/open/close, legacy manual deposit, signed balance adjustment, dealer cash out, payout event, payout acknowledgment, cashout retry, and payout-operation reconcile. A failed logical operation retains that exact key for HTTP or dealer retry. Payout event keys are deterministic from `operationId` and `sequenceNumber`; acknowledgments use a separate deterministic purpose; reconcile uses a deterministic per-`operationId` key so repeated recovery attempts for the same stranded operation are idempotent. Invite creation is intentionally non-financial and non-idempotent because its opaque single-use secret must be stored hash-only and a lost successful response cannot safely reproduce the raw token. No trade observation creates a deposit.
...
A physically completed outgoing trade is never re-run. The plugin only starts a leg when its durable outbox holds no unsent event for that operation, and it never moves gil until the leg's `TradeOpened` event is durably persisted, so any trade that could have transferred gil always leaves a durable trace. The `QueuePayoutLeg` push is the fast next-leg trigger; the poll and reconnect paths replay the outbox and then resume open operations from `GET /payout-operations/open` as a backend-driven fallback. An operation the backend still reports as `tradeOpened`/`tradeLocked` (physically opened) whose executor can no longer drive it is handed to `POST /payout-operations/{operationId}/reconcile` rather than re-traded: the backend quarantines that uncertain leg without refunding it, cancels and refunds only the definitely-untraded later legs, and flags the membership for in-game verification.
